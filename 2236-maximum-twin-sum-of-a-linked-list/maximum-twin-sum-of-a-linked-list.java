...
The two-pointer method beats the ArrayList method due to three main hidden bottlenecks:

1. The Cost of Dynamic Resizing (The Biggest Culprit)When you create 
List<Integer> list = new ArrayList<>();
Java creates a small internal array with a default size (usually room for just 10 elements).As your while loop runs and adds thousands of items, that small array fills up.When it gets full, Java has to pause your code, create a brand-new, larger array, copy every single item from the old array into the new one, and then delete the old one.This dynamic resizing and data copying happens multiple times for large inputs, slowing your runtime significantly.
2. Auto-Boxing (Object Overhead)Your linked list stores primitive numbers 
(ListNode.val is an int). However, a Java ArrayList cannot hold primitives; it can only hold objects.When you call list.add(curr.val), Java silently converts your lightweight int into a heavy Integer object. This hidden process is called auto-boxing.Creating thousands of new Integer objects on the fly wastes precious CPU cycles.The two-pointer method bypasses this entirely because it reads the raw int values directly from the nodes without creating any objects.
3. Garbage CollectionBecause the ArrayList version allocates a lot of memory for the list and all those boxed Integer objects, it leaves behind a massive trail of temporary data.Java's Garbage Collector has to actively jump in, pause or throttle your program, and clean up that deleted memory.The two-pointer method uses \(O(1)\) auxiliary space. It uses almost zero extra memory, meaning the Garbage Collector has absolutely nothing to clean up.
*/
